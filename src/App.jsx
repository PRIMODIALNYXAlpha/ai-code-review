...
const App = () => {
  ...
  async function reviewCode() {
    ...
    try {
      const res = await fetch("https://api.deepseek.ai/v1/code-review", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          code: code,
          instructions: `You are an expert-level software developer. Review the code and provide:
1️⃣ A quality rating (Better, Good, Normal, Bad)
2️⃣ Detailed suggestions and best practices
...
6️⃣ Fixes and recommendations`
      ...
      setResponse(data.review || "No response from DeepSeek AI.");
    } catch (err) {
      console.error(err);
      setResponse("Error connecting to DeepSeek AI.");
    }
  ...
  async function fixCode() {
    ...
    try {
      const res = await fetch("https://api.deepseek.ai/v1/code-fix", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${DEEPSEEK_API_KEY}`
        ...
        body: JSON.stringify({
          language: selectedOption.value,
          code: code,
          instructions: `You are an expert-level software developer. Analyze the code and provide a fully fixed, optimized, and clean version, explaining your changes.`
        })
      });

      const data = await res.json();
      setResponse(data.fixedCode || "No response from DeepSeek AI.");
    } catch (err) {
      console.error(err);
      setResponse("Error connecting to DeepSeek AI.");
    }
